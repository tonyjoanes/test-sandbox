...
| File | Purpose |
|---|---|
| [`templates/steps/build-and-test.yml`](templates/steps/build-and-test.yml) | Step template — dedupes a restore/build/test sequence, parameterised by SDK version and config |
| [`templates/jobs/run-tests.yml`](templates/jobs/run-tests.yml) | Job template — a whole test job, stamped out multiple times with different parameters |
| [`templates/stages/deploy-stage.yml`](templates/stages/deploy-stage.yml) | Stage template — one deployment stage, invoked once per environment (dev/staging/production) |

Templates are the reuse mechanism at three different granularities: use a
**step** template to dedupe a repeated sequence of commands, a **job**
template when the reusable unit needs its own pool/strategy, and a **stage**
template when whole phases (like "deploy to an environment") repeat with
only the environment name changing. File `10` shows all three used together.
...
4. **Templates, then 10**: once the individual concepts make sense, see how they compose into one pipeline you'd actually put in a repo.
...
- **Template** — a separate YAML file included via `template:`, parameterised with `parameters:`. Three flavours: step templates, job templates, stage templates (see table above).
...
3. For file `10` specifically, the `template:` references are relative paths (`../templates/...`) — keep the `templates/` folder alongside `pipelines/` if you copy things out, or just point the pipeline definition at file 10 in place.
